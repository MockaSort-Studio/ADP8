...
struct find_table_item<TargetKey, First, Rest...> {
  using type =
      std::conditional_t<std::is_same_v<TargetKey, typename First::Key>, First,
...
struct find_table_item<TargetKey, Last> {
  using type = std::conditional_t<std::is_same_v<TargetKey, typename Last::Key>,
                                  Last, void>;
};
...
struct LookupTable : public TableElements... {
  ...
  static constexpr std::size_t count_key_v =
      (0 + ... +
       (std::is_same_v<TargetKey, typename TableElements::Key> ? 1 : 0));

  static_assert(((count_key_v<typename TableElements::Key> == 1) && ...),
  ...
  // that the order of default values must match the order of declaration for
  // the table items
  ...
  // Use this for complex cases, where the number of entry is > 10 and your
  // types are heavely using templates tag dispatchig saves us from the
  // ::template syntax.
  ...
  constexpr const auto& GetValue() const {
    using ItemBase =
        typename find_table_item<TargetKey, TableElements...>::type;
    return static_cast<const ItemBase&>(*this).values;
  }

  // Sometimes we end up in dependent template types
  // this is a small helper to enable tag dispatching and auto parameter
  // deduction so we avoid stupid syntax table.template GetValue<Tag>() oink
  // oink 🐽
