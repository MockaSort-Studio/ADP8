...
constexpr auto has_visit_nested(int)
    -> decltype(std::declval<T>().visit_nested(std::string_view{}),
                std::true_type{});
...
constexpr auto has_exit_nested(int)
    -> decltype(std::declval<T>().exit_nested(std::string_view{}),
                std::true_type{});
...
/**
 * @brief Base class for visitors. This class uses CRTP pattern to forward
 * function calls to the Derived class
 * @tparam Derived The derived visitor class.
 */
template <typename Derived>
class VisitorBase {
  /**
   * @brief Visit a named value. This is called by the visitable everytime an
   * element is visited.
  ...
  /**
   * @brief Enter a nested visitable object. This is called by the visitable
   * everytime a visitable object (or nested) is visited.
  ...
  /**
   * @brief Exit a nested visitable object. This is called by the visitable
   * everytime a visiting a visitable object (or nested) is over.
   * @param name The name of the nested object.
   * @return Implementation-defined.
   */
