...
#ifndef SUPPORT_VISIT_VISITABLE
#define SUPPORT_VISIT_VISITABLE
 ...
 */
template <typename T>
inline constexpr bool is_i_visitable_v =
    std::is_base_of_v<IVisitable, std::decay_t<T>>;

/**
 * @brief Type trait to check if a type has a member function called
 * GetVisitable() that returns a type derived from IVisitable.
 * @tparam T The type to check.
 */
...
/**
 * @brief Type trait that checks if a type has a static member function
 * GetVisitable() that returns an object derived from IVisitable.
 *
 * @tparam T The type to check.
 */
template <typename T>
struct has_get_visitable<
    T, std::enable_if_t<is_i_visitable_v<decltype(T::GetVisitable())>>>
    : std::true_type {};

/**
 * @brief Class to make a type visitable.
 * Look at tests for usage examples. This class
 * creates a visitable object that defines how the instance of an object should
 * be visited.
...
class Visitable : public IVisitable {
 public:
  ...
  /**
   * @brief Accepts a visitor and triggers the visiting chain by calling the
   * Visitor callbacks when with the instance params.
  ...
  void accept(Visitor&& visitor, InstanceT& instance) {
    ...
    visit_foreach_property(std::forward<Visitor>(visitor), instance,
                           std::make_index_sequence<prop_size>{});
  }

 private:
  template <typename Visitor, typename PropertyT>
  void visit_property(Visitor&& visitor, InstanceT& instance,
                      const PropertyT& prop) {
  ...
  void visit_foreach_property(Visitor&& visitor, InstanceT& instance,
