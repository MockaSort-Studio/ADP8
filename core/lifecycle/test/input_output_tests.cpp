...
{
    constexpr uint64_t hash_1 = Hash(gen::kSendFalseTopicName);
    constexpr uint64_t hash_2 = Hash(gen::kSendFalseTopicName);
    constexpr uint64_t hash_3 = Hash(gen::kSendTrueTopicName);
...
TEST(TestInputOutput, SameTypeSpecializedWithDifferentTopic)
{
    ...
    auto& input_1 = get<gen::kSendFalseTopicName>(inputs);
    auto& input_2 = get<gen::kSendTrueTopicName>(inputs);
    auto& output_1 = get<gen::kSendFalseTopicName>(outputs);
    auto& output_2 = get<gen::kSendTrueTopicName>(outputs);
    ...
    EXPECT_EQ(input_1.kHash, Hash(gen::kSendFalseTopicName));
    EXPECT_EQ(input_2.kHash, Hash(gen::kSendTrueTopicName));
    ...
    EXPECT_EQ(output_1.kHash, Hash(gen::kSendFalseTopicName));
    EXPECT_EQ(output_2.kHash, Hash(gen::kSendTrueTopicName));
}
...
TEST(TestInputOutput, TestSetGet)
{
    ...
    auto& producer = get<gen::kSendFalseTopicName>(outputs);
    auto& consumer = get<gen::kSendFalseTopicName>(inputs);
