...
class CarSimulationNode : public sert::core::TaskInterface {
 public:
  CarSimulationNode(const std::string& name, rclcpp::NodeOptions options)
      : TaskInterface(name, options.append_parameter_override(
                                "cycle_time_ms", T_SIMULATION_PUB)) {
    ...
    RegisterSubscriber<car_msgs::msg::ActuatorCommands>(
        "actuator_cmd", [this](car_msgs::msg::ActuatorCommands::UniquePtr msg) {
 ...
 private:
  ...
  void updateActuators() {
    ...
    act_state_.motor_speed +=
        (DT / MOTOR_TAU) * (v_target - act_state_.motor_speed);
