        ...
        with open(parameter_file_path, 'r') as file:
            ...
            for field in fields(RuntimeParams):
                if field.name not in params:
                    raise KeyError(f"Parameter file {parameter_file_path}: Missing required parameter: {field.name}")
            # Filter out extra keys
            filtered_params = {field.name: params[field.name] for field in fields(RuntimeParams)}
