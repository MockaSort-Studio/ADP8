...
class TeleOpNode : public sert::core::TaskInterface {
 public:
  TeleOpNode(const std::string& name, rclcpp::NodeOptions options)
      : TaskInterface(name, options.append_parameter_override("cycle_time_ms",
                                                              T_TELEOP_PUB)) {
    ...
    if (input_fd_ < 0) {
      RCLCPP_ERROR(this->get_logger(), "Failed to open /dev/input/event6: %s",
                   strerror(errno));
 ...
 private:
  void readKeyboardEvents() {
    ...
    while (read(input_fd_, &ev, sizeof(ev)) == sizeof(ev)) {
      ...
      switch (ev.code) {
        case KEY_UP:
          if (pressed) cmd_.v = V_MAX;
          if (released) cmd_.v = 0.0f;
          break;
        ...
        case KEY_LEFT:
          if (pressed) cmd_.d = D_MAX;
          if (released) cmd_.d = 0.0f;
          break;
