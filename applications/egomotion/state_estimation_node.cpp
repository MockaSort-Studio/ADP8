 ...
 public:
  StateEstimationNode(const std::string& name, rclcpp::NodeOptions options)
      : TaskInterface(name, options.append_parameter_override(
                                "cycle_time_ms", T_STATEESTIMATION_PUB)) {
    RegisterSubscriber<car_msgs::msg::IMURaw>(
        "imu_raw", [this](car_msgs::msg::IMURaw::UniquePtr msg) {
          imu_msg_ = *msg;
          imu_received_ = true;
        });
    ...
    RegisterPublisher<car_msgs::msg::CarState>("state_est",
                                               QS_STATEESTIMATION_PUB);
