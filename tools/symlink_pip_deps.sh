...
for REPO_PATH in "$EXTERNAL_ROOT"/rules_python[+][+]*; do
    ...
    # We look for the python toolchain repositories (usually ending in _host or _target)
    if [[ "$REPO_NAME" == *"python_"*"_host"* ]] || [[ "$REPO_NAME" == *"python_"*"_target"* ]]; then
        PYTHON_BIN="$REPO_PATH/bin/python3"
        if [ -f "$PYTHON_BIN" ]; then
            echo "📝 Found Hermetic Interpreter: $REPO_NAME"
            echo "   └─ Mapping binary to $DEST_DIR/bin/python"
            
            ln -sf "$PYTHON_BIN" "$DEST_DIR/bin/python"
...
echo -e "\n✅ IDE sync complete."
echo "   Interpreter path: ${PWD}/${DEST_DIR}/bin/python"
