...

{
 ...
 protected:
  void ExecuteStep() override {
    ...
    RCLCPP_INFO(rclcpp::get_logger("MockaPublisher"), "Publishing: '%s'",
                message.data.c_str());
 ...
 public:
  MockaSubscriber(const std::string& name, rclcpp::NodeOptions options)
    ...
    RegisterSubscriber<std_msgs::msg::String>(
        "topic", [](std_msgs::msg::String::UniquePtr msg) {
          RCLCPP_INFO(rclcpp::get_logger("MockaSubscriber"), "Received: '%s'",
                      msg->data.c_str());
        });
  }
};
