...
```sh
bazel run //test/ros2_bazel/chatter
```
...
```sh
bazel run //test/ros2_bazel/chatter:topic -- echo /topic
```

and list the parameters with

```sh
bazel run //test/ros2_bazel/ros2_param -- list
```

You can record a topic with

```sh
bazel run //test/ros2_bazel/chatter:bag -- record /topic
```

You can run tests with

```sh
bazel test //test/ros2_bazel/chatter:tests
```

Alternatively, you can run nodes without the launch mechanism. In a terminal run

```sh
bazel run //test/ros2_bazel/chatter:talker
```

for rclcpp version or

```sh
bazel run //test/ros2_bazel/chatter:py_talker
```
...
```sh
bazel run //test/ros2_bazel/chatter:listener
```